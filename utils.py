...
def check_accuracy(sess, dset, x, scores, seq_mask,
                   ...
                   labels, is_training=None, final_test=False):
    '''
    ...
    return np.array([acc, prec, true_neg_rate])
...
def train(model_init_fn, optimizer_init_fn, max_seq_length, num_feats,
          ...
          patient_batches_test_final=None):
    '''
          ...
          for (patients, patient_labels) in patient_batches_train:
            ...
            if t % print_every == 0:
              ...
              perf = check_accuracy(sess, patient_batches_test, x, scores, seq_mask,
                                    ...
                                    final_test=False)
              perf_history.append(perf.mean())
              print()
            t += 1

      metrics_final = None
      if patient_batches_test_final != None:
          metrics_final = check_accuracy(sess, patient_batches_test_final, x, scores, seq_mask,
                                         batch_size, max_seq_length, num_feats, feats,
                                         drop_cols, labels, is_training=is_training,
                                         final_test=True)

    return loss_history, perf_history, metrics_final
